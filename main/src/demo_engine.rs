...
use billy_engine::engine::{BillyEngine, Point, Triangle};
...
pub fn demo_triangle(engine: Arc<Mutex<BillyEngine>>) {
    ...
    let terminal_resolution: String = format!(
        "V0.0.1A   R{}X{}",
        tmp_engine.get_resolution().0,
        tmp_engine.get_resolution().1
    );
...
pub fn info(engine: Arc<Mutex<BillyEngine>>) {
    ...
    let terminal_resolution: String = format!(
        "V0.0.1A   R{}X{}",
        engine.get_resolution().0,
        engine.get_resolution().1
    );
    let terminal_resolution: &str = terminal_resolution.as_str();
    let v = Point::new(1, 0);
    engine.clear('-');
