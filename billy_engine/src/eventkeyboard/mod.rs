pub mod eventkeyboard {
    ...
    lazy_static! {
        static ref REGISTERY_KEYS: Vec<&'static str> =
            vec![KEYS_UP, KEYS_DOWN, KEYS_LEFT, KEYS_RIGHT, KEYS_ENTER, KEYS_ESC, KEYS_FOCUS];
    }
        ...
        C: ControllerUi + Send + 'static,
    {
        ...
        subscribe(KEYS_UP, move || {
            let mut c = up.lock().unwrap();
            c.key_up();
        });

        subscribe(KEYS_DOWN, move || {
            let mut c = down.lock().unwrap();
            c.key_down();
        });

        subscribe(KEYS_LEFT, move || {
            let mut c = left.lock().unwrap();
            c.key_left();
        });

        subscribe(KEYS_RIGHT, move || {
            let mut c = right.lock().unwrap();
            c.key_right();
        });

        subscribe(KEYS_ENTER, move || {
            let mut c = enter.lock().unwrap();
            c.key_enter();
        });

        subscribe(KEYS_ESC, move || {
            let mut c = focus.lock().unwrap();
            c.key_esc();
        });

        subscribe(KEYS_FOCUS, move || {
